...
echo ================================================
echo   Setup + Build (leve) - RadioAdDetector
echo ================================================
...
if not exist "%ROOT%bin\ffmpeg.exe" (
    ...
    echo Coloque o arquivo nessa pasta e rode novamente.
    exit /b 1
)
...
echo [2/6] Atualizando pip/setuptools/wheel...
...
echo [5/6] Gerando executavel (perfil leve)...
"%PYTHON%" -m PyInstaller ^
  ...
  --add-data "%ROOT%bin\ffmpeg.exe;bin" ^
  "%ROOT%main.py"
if errorlevel 1 (
...
echo [6/8] Garantindo arquivos no pacote...
if not exist "%BUILD_DIR%\_internal\bin" mkdir "%BUILD_DIR%\_internal\bin"
copy /y "%ROOT%bin\ffmpeg.exe" "%BUILD_DIR%\_internal\bin\ffmpeg.exe" >nul
...
echo.
echo IMPORTANTE: este pacote inclui .env com a chave da API.
echo.
